...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[api] (openmeteo) at (0,0) {Open-Meteo API\\(Weather Data)};
    \node[api] (ree) at (3.5,0) {REE API\\(Electricity Price)};

    % Second layer - Dashboard
    \node[box, fill=cyan!10] (dashboard) at (1.75,-2) {Streamlit\\Dashboard\\(UI)};

    % Third layer - API Layer
    \node[box, fill=green!20] (fastapi) at (1.75,-3.8) {FastAPI\\Protocol API\\(:8000)};

    % Fourth layer - Calculation Module
    \node[api] (calculator) at (1.75,-5.5) {Protocol\\Calculator};

    % Fifth layer - Main Control
    \node[box, fill=blue!20, text width=3.5cm] (mainpy) at (1.75,-7.5) {Main Control\\(Integrated\_Project.py)};

    % Sixth layer - Safety Strategy
    \node[api, text width=2.5cm] (safety) at (5.5,-7.5) {Safety\\Strategy};

    % Bottom layer - Arduino
    \node[box, fill=orange!20] (arduino) at (1.75,-9.5) {ESP32 Arduino\\(Firmware)};

    % Sensors
    \node[sensor] (temp) at (-1,-11) {LM35\\Temp Sensor};
    \node[sensor] (current) at (4.5,-11) {ACS712\\Current Sensor};

    % Actuator
    \node[sensor, fill=red!10] (relay) at (1.75,-11.5) {Relay\\(Load Control)};

    % Arrows - Data flow
    \draw[arrow] (openmeteo) -- (dashboard);
    \draw[arrow] (ree) -- (dashboard);
    \draw[arrow] (dashboard) -- node[right, font=\tiny] {user inputs} (fastapi);
    \draw[arrow] (fastapi) -- node[right, font=\tiny] {GET /protocol} (calculator);
    \draw[arrow] (calculator) -- (mainpy);
    \draw[arrow] (mainpy) -- node[above, font=\tiny] {check violations} (safety);
    \draw[arrow] (safety) -- (mainpy);
    \draw[arrow] (mainpy) -- node[right, font=\tiny] {Serial (115200 baud)} (arduino);
  ...
  \end{tikzpicture}
  \caption{Software schematic of the prototype showing data flow between modules, external APIs, sensors, and actuators.}
...
\end{equation}
where $i(t) = (V_{\text{ADC}} - 1.65) \times 30$ converts the ACS712's AC-coupled voltage to current, and $\Delta t$ in seconds is tracked by the microcontroller. After accumulating 20 samples (20 ms = one AC period at 50 Hz):
\begin{equation}
